...
\begin{frame}
\frametitle{Best of Übungsblatt 1}
\begin{itemize}
	\item Abgaben: 18 von 21 (85,7\%)
	\item Mindestens 50\% Punkte: 18 von 18 (100\%)
	\item Durchschnittliche erreichte Punktzahl: 11,7
\end{itemize}~\\~\\~\\
18 Tutanden haben wir gefragt:~\\
Was war beim ersten Übungsblatt am verwirrendsten?
\begin{enumerate}
	\item \only<7>{Was muss ich bei \glqq Zeigen Sie...\grqq\ tun?}\only<8->{Zeigen Sie = Beweisen Sie}\only<9->{\begin{itemize} \item Also Widerspruch oder Vollständige Induktion \end{itemize}}
	\item \only<4>{Wie Simuliere ich ein System?}\only<5->{Ein Simulation bedeutet alle Schritte des Systems darzustellen}\only<6->{\begin{itemize} \item Bei \glqq Wenden Sie X an\grqq\ übrigens auch\end{itemize}}
	\item \only<2->{rechtslinear \only<2>{$\supseteq = \subseteq$}\only<3->{$\nsupseteq\neq\subseteq$} regulär\only<2>{?}
	}
\end{enumerate}
\end{frame}
...
\subsection{Eliminierung von $\epsilon$-Übergängen}
\begin{frame}
