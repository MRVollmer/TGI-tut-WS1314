...
\item Für alle $i \in \mathbb{N}_0$ gilt: $xy^iz \in L$
\end{enumerate}
%\end{exampleblock}
...
\begin{frame}
	...
	Behauptung: $L=\{\underline{\hspace{1.5 cm}}\}$ ist nicht regulär. ~\\
	Beweis: Annahme $L$ sei regulär:
	\begin{enumerate}
		\item[]Sei $p \in \mathbb{N}$ wie im Pumping-Lemma
		\item[]Wähle $w$ = \underline{\hspace{1.5 cm}}, $w \in L$, $|w| > p$
		\item[]Beh: $\forall x,y,z: w = xyz, |xy| \leq p, y \neq \epsilon$ gilt: $\exists i \in \mathbb{N}_0: xy^iz \notin L$
		\item[] Bew: $(\forall y$ gilt:)\underline{\hspace{4.5 cm}}
...
\section{Kontexfreie Sprachen}
\subsection{Kontextfreie Sprachen}
\begin{frame}
	\frametitle{Kontextfreie Sprachen}
	\begin{itemize}
		\item Echte Obermenge von regulären Sprachen
		\item Auch Chompsky Typ 2 genannt
		\item Wird von Kontextfreien Grammatiken $G = (\Sigma, V, S, P)$ gesprochen
		\begin{description}
			\item[$\Sigma$] endliches Alphabet der Terminalsymbole
			\item[$V$] endliches Alphabet der Variablen ($V \cap \Sigma = \emptyset$)
			\item[$S$] Startsymbol $\in V$
			\item[$P$] Produktionsmenge, mit Produktionsform $u \rightarrow v$, $u \in V$ und $v \in \{\Sigma \cap V\}^*$
		\end{description}
		\item Können von Kellerautomaten erkannt werden
	\end{itemize}
\end{frame}
...
Um das leere Wort in der Sprache zu erlauben, lässt sich die Grammatik leicht mit neuem Startsymbol $S'$ ergänzen mit der Regel $$ S' \rightarrow S \mid \epsilon $$
\end{exampleblock}
}
...
X &\rightarrow \textcolor{red}{a}X\textcolor{red}{b} \mid Z \mid \epsilon\\
Y &\rightarrow \textcolor{red}{cc}Y \mid \epsilon\\
Z &\rightarrow X\\
\end{align*}
\end{column}
%
...
\begin{align*}
\mathcal{S} &\rightarrow XY\\
X &\rightarrow \textcolor{blue}{A}X\textcolor{blue}{B} \mid Z \mid \epsilon\\
Y &\rightarrow \textcolor{blue}{CC}Y \mid \epsilon\\
Z &\rightarrow X\\
...
\begin{align*}
\mathcal{S} &\rightarrow XY\\
X &\rightarrow \textcolor{red}{AXB} \mid Z \mid \epsilon \\
Y &\rightarrow \textcolor{red}{CCY} \mid \epsilon \\
Z &\rightarrow X\\
A &\rightarrow a\\
B &\rightarrow b\\
C &\rightarrow c\\
...
\begin{align*}
\mathcal{S} &\rightarrow XY\\
X &\rightarrow \textcolor{blue}{FB} \mid Z \mid \epsilon \\
Y &\rightarrow \textcolor{blue}{GY} \mid \epsilon \\
Z &\rightarrow X\\
...
\begin{enumerate}
\setcounter{enumi}{2}
\item Entfernen von Produktionen der Form $\textcolor{red}{V} \rightarrow \textcolor{red}{\epsilon}$
...
\begin{align*}
\mathcal{S} &\rightarrow XY\\
\textcolor{red}{X} &\rightarrow FB \mid Z \mid \textcolor{red}{\epsilon}\\
\textcolor{red}{Y} &\rightarrow GY \mid \textcolor{red}{\epsilon}\\
Z &\rightarrow X\\
F &\rightarrow AX\\
G &\rightarrow CC\\
...
\begin{column}{0.3\textwidth}
\begin{align*}
\textcolor{red}{\mathcal{S}} &\rightarrow XY \mid \textcolor{blue}{X} \mid  \textcolor{blue}{Y} \mid \textcolor{blue}{\epsilon}\\
\textcolor{red}{X} &\rightarrow FB \mid Z \mid \textcolor{blue}{\msout{\epsilon}}\\
\textcolor{red}{Y} &\rightarrow GY \mid \textcolor{blue}{G}\mid \textcolor{blue}{\msout{\epsilon}}\\
...
\begin{column}{0.3\textwidth}
\begin{align*}
\mathcal{S} &\rightarrow XY \mid X \mid Y \mid \epsilon \\
...
Außerdem wird ein neues Startsymbol eingeführt, falls eine Regel $\mathcal{S} \rightarrow \epsilon$ existiert.
\end{enumerate}
...
\begin{column}{0.3\textwidth}
\begin{align*}
\mathcal{S} &\rightarrow XY \mid \textcolor{red}{X} \mid  \textcolor{red}{Y} \mid \textcolor{red}{\epsilon}\\
X &\rightarrow FB\\
...
\begin{column}{0.3\textwidth}
\begin{align*}
\textcolor{blue}{\mathcal{S'}} & \rightarrow \textcolor{blue}{\mathcal{S} \mid \epsilon} \\
...
\begin{enumerate}
	...
	\item Produktionen auf $\epsilon$ ersetzen
	\item Produktionen auf ein Nicht-Terminal ersetzen
	\item Neues Startsymbol einführen falls $S \rightarrow \epsilon$ existiert
	\item ???
	\item PROFIT
\end{enumerate}
}
...
\begin{frame}
	...
	\begin{figure}
		\begin{tikzpicture}[node distance=2.5cm,shorten >=1pt,auto]
			...
			\path[->]	(q_0) 	edge 			node {$(b,X,\epsilon)$}				(q_1)
			edge [loop above]	node {${(a,X,XX)} \atop {(a,\#,X\#)}$}	 	()
			(q_1)	edge			node {$(\epsilon,\#,\epsilon)$}			(q_2)
